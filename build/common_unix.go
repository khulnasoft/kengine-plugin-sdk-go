...
// ReloadPlugin - kills any running instances and waits for khulnasoft to reload the plugin
func ReloadPlugin() error {
	...
	// Wait for khulnasoft to start plugin
	for i := 0; i < 20; i++ {
		...
		log.Printf("waiting for khulnasoft to start: %s...", exeName)
	}
	return nil
}
...
//
// The plugin process is killed after re-building, in order to make Khulnasoft launch the new version. Once the new
...
func Debugger() error {
	...
	for i := 0; i < 20; i++ {
		...
		log.Printf("Waiting for Khulnasoft to start plugin: %q...", exeName)
		time.Sleep(250 * time.Millisecond)
	}
	if pid == -1 {
		return fmt.Errorf("could not find plugin process %q, perhaps Khulnasoft is not running?", exeName)
	}
