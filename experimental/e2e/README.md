...
4. Point Khulnasoft at the proxy by exporting the `HTTP_PROXY` and `HTTPS_PROXY` environment variables:
...
This step is needed so that the proxy can intercept HTTPS traffic. By default, the bundled [certificate](certificate_authority/kengine-e2e-ca.pem) and [private key](certificate_authority/kengine-e2e-ca.key.pem) are used. For more information about using a custom CA key pair, see the [ca_keypair](#ca_keypair) config section.
...
```
mage e2e:certificate > /tmp/kengine-e2e.crt
```

2. Add CA certificate:

```
sudo security add-trusted-cert -d -p ssl -p basic -k /Library/Keychains/System.keychain /tmp/kengine-e2e.crt
```
...
An object used define paths to a custom CA certificate and private key in PEM format. By default, the bundled [certificate](certificate_authority/kengine-e2e-ca.pem) and [private key](certificate_authority/kengine-e2e-ca.key.pem) are used. For more information about generating a custom self-signed CA certificate, see the [Certificate Authority Key Pair Generation](certificate_authority/README.md) documentation.
